...
⚠️ Important Note on BigQuery Datasets: -> Do not change the default dataset names (staging, intermediate, marts) in the .env file. The dbt_project.yml relies on this standard nomenclature to build the semantic layer. 

If you wish to deploy to different dataset names, you must update the +schema configurations directly within dbt/zoomcamp/dbt_project.yml.
